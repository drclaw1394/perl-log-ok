...
Optionally synchronise the logging level of your logger to that configured by C<Log::OK> :
...
Then bring in C<Log::OK>, and optionally configure it with a default
...
If your selected logging system was L<Log::ger>, and the logging level was set to "info"
...
options.  It has a C<":s"> appended to it to allow L<GetOpt::Long> to
process just a switch or a switch with a value.

For example if C<opt=\gt"verbose=s">, the logging level will be set to "debug"
with the following:
...
example setting to C<env=\gt"MY_VAR">  the logging level could be set to
"trace" with:
...
The other way is to process command line options in your script. C<Log::OK>
does not consume the C<@ARGV> when processing command line arguments.
